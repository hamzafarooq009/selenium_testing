# html-compare - the diff that understands HTML

This package is forked from [hiff] package and modified to show all of changes
...
html-compare uses heuristics to guess whether a node is unchanged, edited, or completely replaced with a new one. This can be the difference between one "changed" in your diff (if the nodes in the old and new DOM are still deemed to be similar enough) and an "added"/"removed" pair (if html-compare guesses they're not the same node).
...
This lets you influence the weights (relative to defaults) that html-compare will give to each of the four components when comparing tags: the **name** of the tag, the **id** attribute (which is treated separately from other attributes as important for identifying nodes), other **attributes**, and the **contents** of the tag (children and text). The default settings is `1` for each property. Setting it to a higher number will make this component more important, and to a lower number - less important. Setting it to 0 will make html-compare **completely ignore** the given component.
...
```javascript
// this will make html-compare treat <div>Blah</div> and <section>Blah</section> as identical
...
[hiff]: https://github.com/krajzeg/hiff